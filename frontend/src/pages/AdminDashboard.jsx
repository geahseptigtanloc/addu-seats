...
export default function AdminDashboard() {
  ...
  return (
    <Layout>
      ...
      <section className="grid gap-4 py-6 sm:grid-cols-2 xl:grid-cols-4" aria-label="Occupancy summary">
      ...
      <section className="ui-panel overflow-hidden">
        <div className="flex items-center justify-between gap-4 border-b border-slate-200 bg-white/95 px-5 py-4">
          <div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-[720px] text-left text-sm">
            <thead className="border-b border-slate-200 bg-[#f5f9fc] text-xs uppercase text-slate-500">
              <tr>
            ...
            <tbody className="divide-y divide-slate-100">
              {FLOOR_DATA.map((item) => (
                <tr key={item.floor} className="hover:bg-blue-50/40">
                  <td className="px-5 py-4">
                    <div className="flex items-center gap-3 font-semibold text-slate-950">
                      <span className="grid h-8 w-8 place-items-center rounded-[8px] bg-[#e6f0f7] text-xs text-[#063a64]">{item.floor}</span>
                      Floor {item.floor}
                    </div>
                  </td>
                  ...
                  <td className="px-5 py-4">
                    <div className="flex items-center gap-3">
                      <div className="h-2 w-32 overflow-hidden rounded-sm bg-slate-200" aria-hidden="true">
                        <div className="h-full bg-[#063a64]" style={{ width: `${item.utilization}%` }} />
                      </div>
                      <span className="w-9 text-right text-xs font-semibold text-slate-600">{item.utilization}%</span>
                    </div>
                  </td>
                  <td className="px-5 py-4 text-right">
                    <Link to={`/map/gisbert/${item.floor}`} className="inline-flex items-center gap-2 font-semibold text-[#063a64] hover:underline">
                      Open map
  ...
  return (
    <div className="ui-panel overflow-hidden p-5">
      <div className="flex items-start justify-between gap-4">
        ...
        <span className={`h-10 w-2 rounded-sm ${accents[tone]}`} aria-hidden="true" />
      </div>
    </div>
  );
}
